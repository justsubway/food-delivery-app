import {SafeAreaView} from "react-native-safe-area-context";
import { FlatList, Image, Pressable, Text, TouchableOpacity, View, Alert} from "react-native";
import {Fragment, useState} from "react";
import cn from 'clsx';
import { router } from "expo-router";

import CartButton from "@/components/CartButton";
import {images, offers} from "@/constants";
import useAuthStore from "@/store/auth.store";
import useLocationStore from "@/store/location.store";

export default function Index() {
  const { user } = useAuthStore();
  const { currentLocation, setLocation } = useLocationStore();

  const handleLocationPress = () => {
    Alert.prompt(
      "Change Location",
      "Enter your city and state:",
      [
        {
          text: "Cancel",
          style: "cancel"
        },
        {
          text: "Update",
          onPress: (text) => {
            if (text && text.trim()) {
              setLocation(text.trim());
            }
          }
        }
      ],
      "plain-text",
      currentLocation
    );
  };

  return (
      <SafeAreaView className="flex-1 bg-white">
          <FlatList
              data={offers}
              renderItem={({ item, index }) => {
                  const isEven = index % 2 === 0;

                  return (
                      <View>
                          <Pressable
                              className={cn("offer-card", isEven ? 'flex-row-reverse' : 'flex-row')}
                              style={{ backgroundColor: item.color }}
                              android_ripple={{ color: "#fffff22"}}
                              onPress={() => {
                                  // Navigate to search page with appropriate category filter
                                  const categoryMap: { [key: string]: string } = {
                                      "SUMMER COMBO": "Burgers",
                                      "BURGER BASH": "Burgers", 
                                      "PIZZA PARTY": "Pizzas",
                                      "BURRITO DELIGHT": "Burritos"
                                  };
                                  const category = categoryMap[item.title] || "All";
                                  router.push({
                                      pathname: "/search",
                                      params: { category }
                                  });
                              }}
                          >
                              {({ pressed }) => (
                                  <Fragment>
                                      <View className={"h-full w-1/2"}>
                                        <Image source={item.image} className={"size-full"} resizeMode={"contain"} />
                                      </View>

                                      <View className={cn("offer-card__info", isEven ? 'pl-10': 'pr-10')}>
                                          <Text className="h1-bold text-white leading-tight">
                                              {item.title}
                                          </Text>
                                          <Image
                                            source={images.arrowRight}
                                            className="size-10"
                                            resizeMode="contain"
                                            tintColor="#ffffff"
                                          />
                                      </View>
                                  </Fragment>
                              )}
                          </Pressable>
                      </View>
                  )
              }}
              contentContainerClassName="pb-28 px-5"
              ListHeaderComponent={() => (
                  <View className="flex-between flex-row w-full my-5">
                      <View className="flex-start">
                          <Text className="small-bold text-primary">DELIVER TO</Text>
                          <TouchableOpacity 
                              className="flex-center flex-row gap-x-1 mt-0.5"
                              onPress={handleLocationPress}
                          >
                              <Text className="paragraph-bold text-dark-100">{currentLocation}</Text>
                              <Image source={images.arrowDown} className="size-3" resizeMode="contain" />
                          </TouchableOpacity>
                      </View>

                      <CartButton />
                  </View>
              )}
          />
      </SafeAreaView>
  );
}
